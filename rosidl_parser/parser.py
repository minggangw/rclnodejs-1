...
def get_json_object_from_msg_spec_object(spec_object):
    fields = []
    for field in spec_object.fields:
        dict_field = {'name': field.name}
        dict_field['type'] = get_json_object_from_type_object(field.type)
        fields.append(dict_field)

    constants = []
    for constant in spec_object.constants:
        constants.append({'type': constant.type, 'name': constant.type, 'value': constant.value})

    msg_name = {'msgName': spec_object.msg_name}
    json_obj = {'constants': constants, 'fields': fields, 'baseType': get_json_object_from_base_type_object(spec_object.base_type),
        'msgName': spec_object.msg_name}
